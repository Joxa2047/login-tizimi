...
const elSearch_input = document.querySelector(".pokemon__search-input")
function render(arr, list){
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for(let j = 0; j < arr[i].type.length; j++){
            const elType_f = document.createElement("p")
            elType_f.innerHTML = arr[i].type[j]

            elType_f.setAttribute("class", "pokemon__item-type_1")
            elType.appendChild(elType_f)
        }

        elItem.setAttribute("class", "pokemon__item")
        elId.setAttribute("class", "pokemon__item-id")
        elImg.setAttribute("class", "pokemon__item-img")
        elName.setAttribute("class", "pokemon__item-name")
        const elHeight = document.createElement("p")
        const elWeight = document.createElement("p")
        elHeight.setAttribute("class", "pokemon__item-info_height")
        elWeight.setAttribute("class", "pokemon__item-info_weight")
        elInfo.appendChild(elHeight)
        elInfo.appendChild(elWeight)
        elInfo.setAttribute("class", "pokemon__item-info")
        elType.setAttribute("class", "pokemon__item-type")

        elId.innerHTML = `№ ${arr[i].id}`
        elImg.setAttribute("href", `${arr[i].img}`)
        elName.innerHTML = arr[i].name
        elHeight.innerHTML = arr[i].height
        elWeight.innerHTML = arr[i].weight
...
elSearch_input.addEventListener("input", () => {
    ...
    const filtered = pokemons.filter((el) =>
        el.name.toLowerCase().includes(value)
    )
    if (filtered.length == 0){
        elList.innerHTML = `<h1 style="color: #d565ff" >Afsus hech nima topilmadi</h1>`
    }
    else{
